...
export const sortOutData = async (data: Claim[], address: string) => {
    ...
    for (let i = 0; i < data.length; i += 50) {
        ...
        const processedBatch = await Promise.all(batch.map(claim => processClaim(claim, address)));
...
const processClaim = async (claim: Claim, address: string) => {
    ...
    if (claim.claimed) {
        ...
        const merkleProof = await getMerkleProof(claim.ipfsHash, address, Number(claim.batchId));
        if (merkleProof && merkleProof.proof.length > 0 && merkleProof.amount !== "0") {
